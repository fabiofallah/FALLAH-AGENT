...
ok(html.includes('VERSÃO 1.0.0 | PATCH 127'),'footer patch 126');
ok(html.includes('PROVA DO MOTOR — PATCH 127'),'proof patch 126');
...
ok(engine.includes("const PATCH_TAG = 'PATCH_127'"),'engine patch tag');
...
ok(engine.includes("'PATCH_127_ARBITRAGE_PIPELINE.log'"),'no legacy runtime log');
ok(pipeline.includes('MAX_HEAVY_LIVE_CYCLES || 5'),'multi-house bootstrap preserved');
ok(pipeline.includes('FALLAH_ENDPOINT_CONCURRENCY) || 3'),'bounded endpoint collection without serializing houses');
console.log('PATCH 127 consolidated regression: OK');
